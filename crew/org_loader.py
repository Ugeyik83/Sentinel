...
class OrgLoader:
    def __init__(self, org_chart_path: str, job_descriptions: dict = None):
        ...
        self.agents = {}
        self.llm = get_llm()  # artık string: "gpt-4o"
    ...
    def _create_agent(self, node: dict, parent_role: str = None) -> Agent:
        ...
        backstory = self._build_backstory(
            role=role, domains=domains, authority=authority,
            level=level, is_vacant=is_vacant,
            job_desc=job_desc, parent_role=parent_role,
        )
        ...
        return Agent(
            ...
            backstory=backstory,
            llm=self.llm,          # string: "gpt-4o"
            verbose=True,
            allow_delegation=(level <= 1),
            max_iter=5,
        )
    ...
    def _build_goal(self, role: str, domains: list, authority: str, is_vacant: bool) -> str:
        ...
        if is_vacant:
            return (
                f"[VEKİL] {role} pozisyonu boş. "
                f"{domain_str} alanında en iyi kararı savun."
            )
        return (
    ...
    def _build_backstory(self, role, domains, authority, level,
                         is_vacant, job_desc, parent_role) -> str:
        base = f"Sen {role} pozisyonundasın."
        if parent_role:
            base += f" {parent_role}'e raporluyorsun."
        if is_vacant:
            base += " Bu pozisyon şu an boş — vekil olarak görev yapıyorsun."
        if job_desc:
            base += f" Görev tanımın: {job_desc[:300]}"
        else:
            base += f" Temel sorumlulukların: {', '.join(domains)}."
        if level == 0:
            base += " Nihai karar merciisin."
        elif level == 1:
            base += " Kendi alanında stratejik kararlar alırsın."
        else:
            base += " Operasyonel kararlar alır, direktörüne raporlarsın."
        return base

    def build_management_committee(self) -> list:
        committee_ids = self.chart["simulation_config"]["consensus_roles"]
        return [self.agents[aid] for aid in committee_ids if aid in self.agents]
...
class ScenarioTaskBuilder:
    def build_tasks(self, scenario: dict, agents: dict) -> list:
        ...
        level2_tasks = self._build_operational_tasks(scenario_text, time_horizon, agents)
        tasks.extend(level2_tasks)

        level1_tasks = self._build_director_tasks(scenario_text, time_horizon, agents, level2_tasks)
        tasks.extend(level1_tasks)

        md_task = self._build_md_task(scenario_text, time_horizon, agents, level1_tasks)
    ...
    def _build_operational_tasks(self, scenario, horizon, agents) -> list:
        operational_roles = [
            "finance_control_manager", "internal_control_risk_manager",
            "it_manager", "quality_manager", "purchasing_manager",
        ]
        tasks = []
        for role_id in operational_roles:
            ...
            tasks.append(Task(
                ...
                expected_output="Yapılandırılmış analiz: etki özeti, 3 risk, aksiyon önerileri.",
            ))
        return tasks

    def _build_director_tasks(self, scenario, horizon, agents, upstream_tasks) -> list:
        director_roles = [
            "director_factory_operations", "director_planning_logistics",
            "director_business_services_cfo", "director_sales_marketing", "director_rd",
        ]
        tasks = []
        for role_id in director_roles:
            ...
            tasks.append(Task(
                description=(
                    f"Senaryo: {scenario}\n\n"
                    f"Departman risk özeti hazırla ({horizon} gün):\n"
                    f"1. Departman etkisi (düşük/orta/yüksek/kritik)\n"
                    f"2. Öncelikli 2 aksiyon\n"
                    f"3. MD'ye eskalasyon gerekiyor mu?"
                ),
                agent=agents[role_id],
                expected_output="Departman risk özeti: etki seviyesi, 2 aksiyon, eskalasyon.",
    ...
    def _build_md_task(self, scenario, horizon, agents, upstream_tasks) -> Task:
        ...
        return Task(
            description=(
                ...
                f"2. Öncelikli 3 aksiyon\n"
                f"3. Her aksiyonun sorumlu direktörü\n"
                f"4. YK'ya iletilecek özet (max 5 madde)"
            ),
            agent=md_agent,
            expected_output="Yönetim kararı: risk skoru, 3 aksiyon + sorumlu, YK özeti.",
            context=upstream_tasks,
        )