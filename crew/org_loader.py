...
# PersonaBuilder lazy import — build_agents() içinde yüklenir
...
class OrgLoader:
    ...
    def __init__(self, org_chart_path: str, job_descriptions: dict = None,
        ...
        self._persona_builder = None  # lazy init

    @property
    def persona_builder(self):
        if self._persona_builder is None:
            from crew.persona_builder import PersonaBuilder
    ...
    def _create_agent(self, node: dict, parent_role: str = None) -> Agent:
        ...
        # Persona zenginleştirme
        job_desc = self.job_descriptions.get(node["id"], "")
        history = self.decision_history.get(node["id"], [])
        persona = self.persona_builder.build(node["id"], job_desc, history)
        backstory = self.persona_builder.enrich_backstory(backstory, persona)
